...
os.ttyname(slave)
print
"Create a symbolic link in /dev/ to the above file and use this symlink to connect with MultiWiiConf to help in debugging. Press enter when ready"
# raw_input()
...
def send_cmd(cmd, data):
    ...
    print
    "data hex", map(hex, data_raw)
    print
    "ser_data", map(str, ser_data)
    print
    "ser_data_hex", map(ord, ser_data)
    ser.write(ser_data)
...
def parse_input():
    ...
    while (state != S_END) and (state != S_ERROR):
        ...
        if state == S_HEADER:
            ...
            data_raw = c
            print
            "Data received", map(ord, data_raw)
    ...
    if state == S_END:
        ...
        print
        "input_packet: ", input_packet, map(ord, input_packet)
        data = []
        error = 1
        ser.flushInput()
...
class MultiwiiCopter(object):
    ...
    def send_serial(self, message_type, data):
        ...
        if len(data_raw) != cmd_data_size:
            eprint("Error: data size incorrect for this message type. MSG type: ", message_type, " data size:",
                   len(data), " expected data size:", cmd_data_size)
            return (-1)
...
class Copter(object):
    ...
    def update_vel(self):
        ...
        print
        "Vel:", self.vel, "Acc:", self.acc_comp,
